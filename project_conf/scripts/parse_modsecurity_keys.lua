...
local modsecurity_attributes_modify_string = function (orig_string)
  local result = {}
  local isFirstIteration = true
  local modsecurity_text = orig_string:match("ModSecurity:(.-)%[")
  for match in orig_string:gmatch("%[(.-)%]") do
    if isFirstIteration then
      key = match
      value = modsecurity_text
      isFirstIteration = false
      value = value:gsub("^%s*(.-)%s*$", "%1")
      formattedKey = "\"" .. key .. "\":\"" .. value .. "\""
    else
      key, value = match:match("(%S+)%s+(%S+)")
      key = key:gsub("\"", "")
  ...
  final_string = table.concat(result, ", ")
  return final_string
end
...
function parse_modsecurity_keys (_, timestamp, record) --luacheck: ignore
  if (record["message"] ~= nil and string.find(record["message"], "ModSecurity")) then
    record["modsecurity"] = modsecurity_attributes_modify_string(record["message"])
  end
